...
// Richer, more premium palette — deeper forest, warmer gold accent
const colors = {
  forest: "#16301F",
  forestDeep: "#0F241A",
  moss: "#7FA36A",
...
export default function LoginPage(): JSX.Element {
  ...
  // Separate handler for Google — kept independent from the email/password
  // flow so its own loading state never blocks or gets blocked by the form.
  const handleGoogleSignIn = async () => {
    setGoogleLoading(true);
    try {
      // Real Better Auth social sign-in:
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* Left — branded visual panel with glassmorphism accent */}
      <div
      ...
      >
        <div className="absolute inset-0">
          <Image
            src="https://images.unsplash.com/photo-1602526429747-ac387a91d43b?q=80&w=1200&auto=format&fit=crop"
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(160deg, rgba(15,36,26,0.9) 0%, rgba(15,36,26,0.98) 60%, rgba(9,22,15,1) 100%)",
            }}
          />
          {/* subtle glow accent */}
          <div
            className="absolute -top-24 -right-24 w-96 h-96 rounded-full"
            style={{ background: "radial-gradient(circle, rgba(201,146,47,0.18) 0%, transparent 70%)" }}
          />
        </div>
        ...
        >
          <div
            className="glass-card inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full mb-6 text-[12px] font-medium"
            style={{ color: colors.bambooTan }}
          >
            <Sparkles size={13} />
            Welcome back
          </div>
          <h2 className="brand-font text-3xl xl:text-4xl leading-[1.15] mb-4" style={{ color: colors.cream }}>
            Every login supports a
            <br />
            weaver&apos;s livelihood.
          </h2>
          <p className="text-[14.5px] leading-relaxed max-w-sm" style={{ color: "rgba(250,247,240,0.6)" }}>
            Track your orders, save favorites, and get first access to new
            pieces from our artisan partners.
          </p>
        </motion.div>
      ...
      <div className="flex items-center justify-center p-6 sm:p-10" style={{ backgroundColor: "#FFFFFF" }}>
        <div className="w-full max-w-sm">
          ...
          >
            ...
            <span className="brand-font text-lg" style={{ color: colors.ink }}>
              BambooCraft
            </span>
          </motion.a>

          <motion.div variants={fadeUp} custom={0} initial="hidden" animate="show">
            <h1 className="brand-font text-3xl mb-2" style={{ color: colors.ink }}>
              Sign in
            </h1>
            <p className="text-[13.5px] mb-8" style={{ color: "rgba(33,31,22,0.55)" }}>
              New here?{" "}
              <a href="/register" className="font-semibold" style={{ color: colors.forest }}>
                Create an account
              </a>
            </p>
          </motion.div>
          ...
          <motion.button
            ...
            type="button"
            onClick={fillDemo}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-[13px] font-semibold mb-6 border border-dashed"
            style={{ borderColor: colors.bambooTan, color: colors.forest, backgroundColor: "rgba(212,180,131,0.1)" }}
          >
            <Sparkles size={14} />
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <motion.div variants={fadeUp} custom={2} initial="hidden" animate="show">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="auth-input w-full pl-10 pr-4 py-3 rounded-xl text-[14px] outline-none border"
                  style={{ borderColor: errors.email ? "#D9534F" : "rgba(0,0,0,0.1)" }}
                />
              </div>
            ...
            <motion.div variants={fadeUp} custom={3} initial="hidden" animate="show">
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-[12.5px] font-medium" style={{ color: colors.ink }}>
                  Password
                </label>
                <a href="/forgot-password" className="text-[12px] font-medium" style={{ color: colors.forest }}>
                  Forgot?
                </a>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="auth-input w-full pl-10 pr-11 py-3 rounded-xl text-[14px] outline-none border"
                  style={{ borderColor: errors.password ? "#D9534F" : "rgba(0,0,0,0.1)" }}
                />
                <button
                  type="button"
                ...
                >
                  {showPassword ? (
                    <EyeOff size={16} color="rgba(33,31,22,0.4)" />
                  ) : (
                    <Eye size={16} color="rgba(33,31,22,0.4)" />
                  )}
                </button>
              </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              disabled={loading}
              className="flex items-center justify-center gap-2 py-3.5 rounded-xl text-[14px] font-semibold mt-2"
              style={{ backgroundColor: colors.forest, color: colors.cream, opacity: loading ? 0.7 : 1 }}
            >
          ...
          <motion.div variants={fadeUp} custom={5} initial="hidden" animate="show" className="flex items-center gap-3 my-6">
            <div className="flex-1 h-px" style={{ backgroundColor: "rgba(0,0,0,0.08)" }} />
            <span className="text-[11.5px]" style={{ color: "rgba(33,31,22,0.4)" }}>
              or continue with
            </span>
            <div className="flex-1 h-px" style={{ backgroundColor: "rgba(0,0,0,0.08)" }} />
          </motion.div>

          {/* Single Google button, properly centered and full-width */}
          <motion.button
            variants={fadeUp}
            custom={6}
            initial="hidden"
            ...
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.97 }}
            className="w-full flex items-center justify-center gap-2.5 py-3 rounded-xl text-[13.5px] font-medium border transition-colors hover:bg-gray-300"
            style={{ borderColor: "rgba(0,0,0,0.1)", color: colors.ink, opacity: googleLoading ? 0.6 : 1 }}
          >
