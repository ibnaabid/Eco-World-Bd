    ...
    <div className="min-h-screen grid lg:grid-cols-2" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="relative hidden lg:flex flex-col justify-between p-12 overflow-hidden" style={{ backgroundColor: colors.forest }}>
        <div className="absolute inset-0">
          <Image src="https://images.unsplash.com/photo-1602526429747-ac387a91d43b?q=80&w=1200&auto=format&fit=crop" alt="Handwoven bamboo basket" fill className="object-cover opacity-20" />
          ...
          <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full" style={{ background: "radial-gradient(circle, rgba(201,146,47,0.18) 0%, transparent 70%)" }} />
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="relative z-10">
          <div className="glass-card inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full mb-6 text-[12px] font-medium" style={{ color: colors.bambooTan }}>
            <Sparkles size={13} /> Welcome back
          </div>
          <h2 className="brand-font text-3xl xl:text-4xl leading-[1.15] mb-4" style={{ color: colors.cream }}>
            Every login supports a <br /> weaver&apos;s livelihood.
          </h2>
          <p className="text-[14.5px] leading-relaxed max-w-sm" style={{ color: "rgba(250,247,240,0.6)" }}>
            Track your orders, save favorites, and get first access to new pieces from our artisan partners.
          </p>
        </motion.div>

        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.4 }} className="relative z-10 text-[12px]" style={{ color: "rgba(250,247,240,0.35)" }}>
      ...
      <div className="flex items-center justify-center p-6 sm:p-10" style={{ backgroundColor: "#FFFFFF" }}>
        <div className="w-full max-w-sm">
          ...
          <motion.div variants={fadeUp} custom={0} initial="hidden" animate="show">
            <h1 className="brand-font text-3xl mb-2" style={{ color: colors.ink }}>Sign in</h1>
            <p className="text-[13.5px] mb-8" style={{ color: "rgba(33,31,22,0.55)" }}>
              New here?{" "}
              <a href="/register" className="font-semibold" style={{ color: colors.forest }}>Create an account</a>
            </p>
          </motion.div>

          <motion.button variants={fadeUp} custom={1} initial="hidden" animate="show" type="button" onClick={fillDemo} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-[13px] font-semibold mb-6 border border-dashed" style={{ borderColor: colors.bambooTan, color: colors.forest, backgroundColor: "rgba(212,180,131,0.1)" }}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <motion.div variants={fadeUp} custom={3} initial="hidden" animate="show">
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-[12.5px] font-medium" style={{ color: colors.ink }}>Password</label>
                <a href="/forgot-password" className="text-[12px] font-medium" style={{ color: colors.forest }}>Forgot?</a>
              </div>
              <div className="relative">
            ...
            <motion.button variants={fadeUp} custom={4} initial="hidden" animate="show" type="submit" whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }} disabled={loading} className="flex items-center justify-center gap-2 py-3.5 rounded-xl text-[14px] font-semibold mt-2" style={{ backgroundColor: colors.forest, color: colors.cream, opacity: loading ? 0.7 : 1 }}>
          ...
          <motion.div variants={fadeUp} custom={5} initial="hidden" animate="show" className="flex items-center gap-3 my-6">
            ...
            <span className="text-[11.5px]" style={{ color: "rgba(33,31,22,0.4)" }}>or continue with</span>
            <div className="flex-1 h-px" style={{ backgroundColor: "rgba(0,0,0,0.08)" }} />
          </motion.div>

          <motion.button variants={fadeUp} custom={6} initial="hidden" animate="show" type="button" onClick={handleGoogleSignIn} disabled={googleLoading} whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }} className="w-full flex items-center justify-center gap-2.5 py-3 rounded-xl text-[13.5px] font-medium border transition-colors hover:bg-gray-50" style={{ borderColor: "rgba(0,0,0,0.1)", color: colors.ink, opacity: googleLoading ? 0.6 : 1 }}>
