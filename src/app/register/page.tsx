...
// Same richer, premium palette as the login page
const colors = {
  forest: "#16301F",
  forestDeep: "#0F241A",
  moss: "#7FA36A",
...
export default function RegisterPage(): JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Real Better Auth call — uncomment and wire this in:
  ...
  // Separate handler for Google — independent from the name/email/password
  // signup flow and its own loading state, so social signup never gets
  // blocked by (or blocks) the regular form validation/loading.
      ...
      >
        <div className="absolute inset-0">
          <Image
            ...
            alt="Handcrafted bamboo furniture"
            fill
          ...
          <div
            ...
            style={{ background: "radial-gradient(circle, rgba(201,146,47,0.16) 0%, transparent 70%)" }}
          />
        </div>
        ...
        >
          ...
          <div className="flex flex-col gap-3">
            {["Early access to new artisan drops", "Save favorites & track orders", "Direct stories from the makers"].map(
              (item, i) => (
                <motion.div
                  key={i}
                  ...
                  className="glass-card flex items-center gap-3 px-3.5 py-2.5 rounded-xl"
                >
                  <span
                    className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                    style={{ backgroundColor: "rgba(212,180,131,0.18)" }}
                  >
                    <Check size={11} color={colors.bambooTan} strokeWidth={3} />
                  </span>
                  <span className="text-[13.5px]" style={{ color: "rgba(250,247,240,0.8)" }}>
                    {item}
                  </span>
                </motion.div>
              )
        ...
        <div className="w-full max-w-sm">
          ...
          <motion.div variants={fadeUp} custom={0} initial="hidden" animate="show">
            ...
            <p className="text-[13.5px] mb-8" style={{ color: "rgba(33,31,22,0.55)" }}>
              Already with us?{" "}
              <a href="/login" className="font-semibold" style={{ color: colors.forest }}>
                Sign in instead
              </a>
            </p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <motion.div variants={fadeUp} custom={1} initial="hidden" animate="show">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  style={{ borderColor: errors.name ? "#D9534F" : "rgba(0,0,0,0.1)" }}
                />
              </div>
            ...
            <motion.label
              ...
              initial="hidden"
              animate="show"
              className="flex items-start gap-2.5 cursor-pointer mt-1"
            >
              <div
                onClick={() => setForm({ ...form, agree: !form.agree })}
                className="custom-checkbox rounded-md border flex items-center justify-center shrink-0 mt-0.5"
                style={{
                  width: 18,
                  height: 18,
                  backgroundColor: form.agree ? colors.forest : "transparent",
                  borderColor: form.agree ? colors.forest : "rgba(0,0,0,0.25)",
                }}
              >
              ...
              <span className="text-[12.5px] leading-snug" style={{ color: "rgba(33,31,22,0.65)" }}>
                I agree to the{" "}
                <a href="/terms" className="font-semibold" style={{ color: colors.forest }}>
                  Terms of Service
                </a>{" "}
                and{" "}
                <a href="/privacy" className="font-semibold" style={{ color: colors.forest }}>
                  Privacy Policy
                </a>
              </span>
            </motion.label>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              disabled={loading}
              className="flex items-center justify-center gap-2 py-3.5 rounded-xl text-[14px] font-semibold mt-1"
              style={{ backgroundColor: colors.ochre, color: colors.cream, opacity: loading ? 0.7 : 1 }}
            >
          ...
          <motion.div variants={fadeUp} custom={7} initial="hidden" animate="show" className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={googleLoading}
              className="flex px-4 w-full items-center justify-center mx-auto gap-2 py-2.5 rounded-xl text-[13px] font-medium border transition-colors hover:bg-gray-300"
